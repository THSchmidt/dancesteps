...
sub getGridVolume {
    ...
    my $nVoxel = 0;
    my $voxelVolume = $gridDeltaX * $gridDeltaY * $gridDeltaZ;
...
#        my $radius  = getElementData($element, 'vdwRadius')*1.4;
        ...
        my $tmpGridX = sprintf("%d", round($$coordDataRef{'atoms'}[$_]{'cooX'} / $gridDeltaX, 1));
        my $tmpGridY = sprintf("%d", round($$coordDataRef{'atoms'}[$_]{'cooY'} / $gridDeltaY, 1));
        my $tmpGridZ = sprintf("%d", round($$coordDataRef{'atoms'}[$_]{'cooZ'} / $gridDeltaZ, 1));
        ########################################################################
        ...
        for (my $z=($tmpGridZ-$subrangeZ); $z<=($tmpGridZ+$subrangeZ); $z++) {
            for (my $x=($tmpGridX-$subrangeX); $x<=($tmpGridX+$subrangeX); $x++) {
                for (my $y=($tmpGridY-$subrangeY); $y<=($tmpGridY+$subrangeY); $y++) {
                    my $dx = $$coordDataRef{'atoms'}[$_]{'cooX'} - $x * $gridDeltaX;
                    my $dy = $$coordDataRef{'atoms'}[$_]{'cooY'} - $y * $gridDeltaY;
                    next if ($dx*$dx + $dy*$dy) > $radius2;
                    my $dz = $$coordDataRef{'atoms'}[$_]{'cooZ'} - $z * $gridDeltaZ;
...
sub grid2CoordData {
    my $gridRef  = shift;
    my $complete = shift; # Convert the entire grid (include also empty grid cells).
    my $voxId   = 0;
    my %gridGroData;
        ...
        for (my $x=$gridXMinAbs; $x<=$gridXMaxAbs; $x++) {
            for (my $y=$gridYMinAbs; $y<=$gridYMaxAbs; $y++) {
                ...
                my %tmpCoords = ('cooX' => $x * $gridDeltaX,
                                 'cooY' => $y * $gridDeltaY,
                                 'cooZ' => $z * $gridDeltaZ);
                push(@{$gridGroData{'atoms'}}, setVoxel(++$voxId, $resName, $resName, $voxId, \%tmpCoords));
            }
        }
    }
    ...
    $gridGroData{'atoms'}  = FileIO::GRO::renumAtoms(\@{$gridGroData{'atoms'}});
...
### The following is old and needs to be checked...
sub getVdwSurf {
    for (my $z=$gridZMin; $z<=$gridZMax; $z++) {
...
#    $gridGroData{'atoms'}  = FileIO::GRO::renumAtoms(\@{$gridGroData{'atoms'}});
#    $gridGroData{'nAtoms'} = scalar(@{$gridGroData{'atoms'}}) - 1;
#    FileIO::GRO::writeGro($gridOutFile, \%gridGroData);
#}
